...
class GroundTruthDetectionDataset(gluon.data.Dataset):
    ...
    """
    def __init__(self, label_path='label', data_path='data', split='train', task='task'):
        """
        Parameters
        ---------
...
def parse_args():
    ...
    parser.add_argument("--task-name", type=str, default='task_name')
    parser.add_argument("--classes-csv", type=str, default='background')
...
'''
    SageMaker automatically uses the function named 'train' as its training blueprint.
'''
def train(task_name, classes, target_epochs, learning_rate): 
    """
    Transfer learning.
    """
    import gluoncv as gcv
    ...
    #
    # Set 'Classes' name to what ever you want it to be.
    #
    model = gcv.model_zoo.get_model('ssd_512_mobilenet1.0_custom', classes=classes, pretrained_base=False, transfer='voc')
    ...
    #define Trainer 
    trainer = gluon.Trainer(model.collect_params(), 'sgd', {'learning_rate': learning_rate, 'wd': 0.0005, 'momentum': 0.9})
    ...
    for epoch in range(0, target_epochs):
...
if __name__ == '__main__':
    ...
    classes = args.classes_csv.split(',')
    
    train(args.task_name, classes, args.target_epochs, args.learning_rate)
